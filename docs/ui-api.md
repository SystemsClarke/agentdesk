...
| `ui:status` | none | `slack` is `slack_bridge.state` as written (`ts`, `poll_s`, `last_relay`), or `null`; up means `ts` is under 90 s old. `concierge` is the `ui:concierge` document. `sessions` is `running` (identities running now) and `max` (the cap, settings.json's `max_sessions`: Options' "Sessions at once"). `usage` is the Claude plan meter from `claude_usage.json` (usage.py): `lines` rotate on the main menu prompt, `summary` is SysOp's Claude plan row. `prs` is the merge list, `pull_requests` rows newest first (200 at most, settled ones included; the window filters). `bridge` is the core's supervision of the Slack bridge (Host/Supervisor.cs): `enabled` (false when `settings.json` has `"slack_bridge": false`), `state`: `external` (a bridge the core did not start, beating), `stale-orphan` (one the core did not start whose pid is alive but has not beaten for 180 s: the core logs a warning and starts no second bridge, which would double-post; kill `orphan_pid` and the core takes over), `supervised`, or `starting`; `supervised`, `orphan_pid`, `pid`, `restarts`, `hung_restarts` (a supervised bridge alive with no heartbeat for 180 s is killed and restarted), `last_exit` and `last_exit_ts`. `governor` is the `ui:governor` document. `goals` is `ui:goal_list`'s rows. | `{"slack": {...}\|null, "concierge": {...}, "sessions": {"running", "max"}, "usage": {"lines": [...], "summary": "..."}, "governor": {...}, "prs": [...], "bridge": {...}, "goals": [...]}` |
| `ui:governor` | none | The usage governor (docs/governor.md): from the last 5 weeks of `usage_samples`, John's forecast burn until the weekly reset, `spendable = remaining - margin - baseline - k*sigma*sqrt(T)`, and the caps and model tiers that would spend it. `enforcing` is settings.json's `governor_enforce` (default false; `advisory` is its opposite). `fresh` is false when the latest sample is over 30 minutes old or `/usage` keeps failing, and `fail_closed` then says why: no new swarm session starts. `baseline_source` is `measured` (session-free hours), `estimated` (the starvation estimate) or `default`. `would_queue` and `would_shed` count, since the core started, the swarm starts and running swarm sessions that enforcement would have queued or shed; `held` names the swarm identities it keeps queued. `series` is the weekly % over the last 7 days, the last reading of each hour, oldest first (the window's budget chart). With no samples: `samples: 0` and a `reason`. | `{"enforcing", "advisory", "fresh", "fail_closed"?, "samples", "baseline_hours", "session_hours", "estimated_hours", "baseline_source", "sample_age_minutes", "used", "remaining", "reset_in_hours", "baseline", "sigma", "reserve", "k", "spendable", "allowed_rate", "session_rate", "projected_end_pct", "five_hour_pct", "caps": {"swarms", "members_per_swarm", "total_sessions", "running", "new_sessions"}, "models": {"step_down", "lead", "member"}, "reason", "series": [...], "summary", "would_queue", "would_shed", "held": [names]}` |
| `ui:governor_enforce` | `on` | John only (the window, Slack `governor on\|off`, the ops console): sets settings.json's `governor_enforce`, keeping its other keys, and logs it. Takes effect at the next session start or governor tick (15 s); no restart. An agent's session gets an error. | the `ui:governor` document |
...
the Concierge, with its on/off toggle (`ui:concierge`); the usage meter and the governor, with its enforce switch (`ui:governor_enforce`); the merge list; open questions; and the core log.
