...
| `ui:identity_create` | `name`, `folder`, `charter?`, `host?`, `autostart?` | Records an identity (board table `identities`): a session that outlives the core. `host` is `windows` (default) or `wsl:<distro>`; `charter` is appended to claude's system prompt. Starts nothing. | the row: `{"name", "folder", "charter", "host", "claude_session_id", "pid", "state", "autostart", "created_ts", "updated_ts"}` |
| `ui:identity_list` | none | Every identity; `state` is `running`, `queued` or `stopped`. | `{"identities": [row, ...]}` |
| `ui:identity_start` | `name` | Runs it as the headless session of the same name: `claude --resume <claude_session_id>` when that conversation has a transcript, else `claude --session-id <id>` (a new id, recorded), plus `--append-system-prompt <charter>`, with `AGENTDESK_IDENTITY` and `AGENTDESK_AUTHOR` set to its name so its board posts carry it. A `wsl:<distro>` host runs `wsl.exe -d <distro> --cd <folder> -- claude ...`, the two variables passed through `WSLENV`. At most `max_sessions` (settings.json, default 3) run at once: past that it is `queued`, and launched, oldest first, when one ends. When the core starts it queues again every identity that was running or queued, and every `autostart` one. | the row |
