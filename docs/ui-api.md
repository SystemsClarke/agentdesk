...
| `ui:identity_create` | `name`, `folder`, `charter?`, `host?`, `autostart?` | Records an identity (board table `identities`): a session that outlives the core. `host` is `windows` (default) or `wsl:<distro>`; `charter` is appended to claude's system prompt. Starts nothing. | the row: `{"name", "folder", "charter", "host", "claude_session_id", "pid", "state", "autostart", "created_ts", "updated_ts", "generation", "phoenix_msg"}` |
