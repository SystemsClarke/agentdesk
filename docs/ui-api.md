...
| `ui:status` | none | The heartbeat files in the data folder, read as the Tk window read them. `slack` is `slack_bridge.state` as written (`ts`, `poll_s`, `last_relay`), or `null`; up means `ts` is under 90 s old. `worker` is `worker.state` plus `running` (its pid is alive), `held` (its `item`, else the newest claimed work item) and `events` (that item's newest 80 `work_events`, oldest first). | `{"slack": {...}\|null, "worker": {...}}` |
...
Not in the core yet, so the window still shows placeholders for them: the PR list and its checker, the Claude usage
meter, the crew roster and Options' backend note, disabled notifier sinks, and the worker start/stop and Wake actions.
