...
/// <summary>
/// Each test is one scenario replayed through C# from golden/seed.db (threads 1-4: two discussion posts, a question
/// John answered with the ack still owed to beta, a claim=anyone work item, and a torch due for builder). Every step's
/// document and the final rows of every table must match what Python recorded. The comments say what each step pins down.
/// </summary>
public class ParityTests
{
    ...
    {
        // Python raised IndexError on a whitespace-only note ("   ".strip().splitlines()[0]), so this golden was
        // recorded from C#; the port uses the url as the title.
        var o = Run([Step("request_merge", "alpha", new { pr_url = "https://github.com/a/b/pull/10", thread_id = 1, note = "   " })]);
        Assert.True((bool)JsonNode.Parse(o.Results[0])!["created"]!);
        using var db = new BoardStore(o.Db).Open();
